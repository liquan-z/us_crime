...
{
  age = read.csv(paste0(year_num,".csv"),skip = 4,header = F,
                 nrows = 51,col.names = c('state','youth_rate'))
  return(age)
}

df_list <- lapply(2010:2019, func)
age_for_regress <- rbind(df_list[[1]],df_list[[2]],df_list[[3]],df_list[[4]]
...
df_for_regress_age <- cbind(year,age_for_regress)

write.csv(df_for_regress_age,'age.csv')
