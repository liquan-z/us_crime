...
#Assign the 2019 crime dataset from github a name
crime2019 <- rio::import('https://github.com/RuofanChen/US-crime-data/raw/main/2019-table-5.xls',sheet = 1, col_names = T,range = "A4:M514")

crime<-crime2019
names(crime)[3] <- 'Report_Type'

# set PUERTO RICO, MASSACHUSETTS pop as 0
crime[388,4] <- '0'
crime[208,4] <- '0'
...
Year<-rep(2019,dim(crime_dfbystate)[1])
crime_df2019 <- data.frame(Year,crime_dfbystate)
...
length(table(crime_df2019$State))

# check number of rows, recheck in line 138
dim(crime_df2019)

# remove number in state name
crime_df2019$State <- gsub('[[:digit:]]+', '', crime_df2019$State )

# Check names
names(crime_df2019)

# give new names for columns
names(crime_df2019) <- c('year','state','area','report_type','actual_rate','violent_crime',
                         'murder','rape','robbery','assault','property_crime',
                         'burglary','larceny_theft','vehicle_theft','population')

# Order the Column's name
crime_df2019 <- crime_df2019[,c('year','state','area','population','actual_rate','report_type',
...
u_crime_df2019 <- gather(crime_df2019,crime_type,number,violent_crime,property_crime,
                         murder,rape,robbery,assault,burglary,larceny_theft,vehicle_theft)

# check number of state
length(table(u_crime_df2019$state))

# recheck number of rows for each crime type  
table(u_crime_df2019$crime_type)


# output the csv file
write.csv(u_crime_df2019,'../../data/1995-2019_reorganized_data/crime_2019.csv')
