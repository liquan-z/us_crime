...
#Assign the 2014 crime dataset from github a name
crime2014 <- rio::import('https://github.com/RuofanChen/US-crime-data/raw/main/2014-table_5_crime_in_the_united_states_by_state_2014.xls',sheet = 1,col_names = T,range = "A4:N509")
crime <- crime2014
...
Year<-rep(2014,dim(crime_dfbystate)[1])
crime_df2014 <- data.frame(Year,crime_dfbystate)
...
length(table(crime_df2014$State))

# check number of rows, recheck in line 138
dim(crime_df2014)

# remove number in state name
crime_df2014$State <- gsub('[[:digit:]]+', '', crime_df2014$State )
# remove , in state name
crime_df2014$State <- gsub('[[:punct:]]+', '', crime_df2014$State)

# Check names
names(crime_df2014)

# give new names for columns
names(crime_df2014) <- c('year','state','area','report_type','actual_rate','violent_crime',
                         'murder','rape','rape_d','robbery','assault','property_crime',
                         'burglary','larceny_theft','vehicle_theft','population')

# Order the Column's name
crime_df2014 <- crime_df2014[,c('year','state','area','population','actual_rate','report_type',
...
u_crime_df2014 <- gather(crime_df2014,crime_type,number,violent_crime,property_crime,
                         murder,rape,robbery,assault,burglary,larceny_theft,vehicle_theft)

# check number of state
length(table(u_crime_df2014$state))

# recheck number of rows for each crime type  
table(u_crime_df2014$crime_type)


# output the csv file
write.csv(u_crime_df2014,'../../data/1995-2019_reorganized_data/crime_2014.csv')
