...
#Assign the 2007 crime dataset from github a name
crime <- rio::import('https://github.com/RuofanChen/US-crime-data/raw/main/2007tbl05.xls',sheet = 1,col_names = T,range = "A4:M499")
...
Year<-rep(2007,dim(crime_dfbystate)[1])
crime_df2007 <- data.frame(Year,crime_dfbystate)
...
length(table(crime_df2007$State))

# check number of rows, recheck in line 138
dim(crime_df2007)

# remove number in state name
crime_df2007$State <- gsub('[[:digit:]]+', '', crime_df2007$State )
# remove , in state name
crime_df2007$State <- gsub('[[:punct:]]+', '', crime_df2007$State)

# Check names
names(crime_df2007)

# give new names for columns
names(crime_df2007) <- c('year','state','area','report_type','actual_rate','violent_crime',
                         'murder','rape','robbery','assault','property_crime',
                         'burglary','larceny_theft','vehicle_theft','population')

# Order the Column's name
crime_df2007 <- crime_df2007[,c('year','state','area','population','actual_rate','report_type',
...
u_crime_df2007 <- gather(crime_df2007,crime_type,number,violent_crime,property_crime,
                         murder,rape,robbery,assault,burglary,larceny_theft,vehicle_theft)

# check number of state
length(table(u_crime_df2007$state))

# recheck number of rows for each crime type  
table(u_crime_df2007$crime_type)


# output the csv file
write.csv(u_crime_df2007,'../../data/1995-2019_reorganized_data/crime_2007.csv')
