...
crime2016 <- rio::import('https://github.com/RuofanChen/US-crime-data/raw/main/2016-table-3.xls',sheet = 1, col_names = T,range = "A4:N505")
...
#write.csv(crime_Metro_df,"crime_metro_2016.csv")
...
Year<-rep(2016,dim(crime_dfbystate)[1])
crime_df2016 <- data.frame(Year,crime_dfbystate)
...
length(table(crime_df2016$State))

# check number of rows, recheck in line 138
dim(crime_df2016)

# remove number in state name
crime_df2016$State <- gsub('[[:digit:]]+', '', crime_df2016$State )

# Check names
names(crime_df2016)

# give new names for columns
names(crime_df2016) <- c('year','state','area','report_type','actual_rate','violent_crime',
                         'murder','rape','rape_d','robbery','assault','property_crime',
                         'burglary','larceny_theft','vehicle_theft','population')

# Order the Column's name
crime_df2016 <- crime_df2016[,c('year','state','area','population','actual_rate','report_type',
...
u_crime_df2016 <- gather(crime_df2016,crime_type,number,violent_crime,property_crime,
                         murder,rape,robbery,assault,burglary,larceny_theft,vehicle_theft)

# check number of state
length(table(u_crime_df2016$state))

# recheck number of rows for each crime type  
table(u_crime_df2016$crime_type)


# output the csv file
write.csv(u_crime_df2016,'../../data/1995-2019_reorganized_data/crime_2016.csv')
