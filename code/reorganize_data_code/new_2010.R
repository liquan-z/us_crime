...
##=======The Raw Dataset miss the PUERTO RICO state ===========##
## Only 51 states ##
...
#Assign the 2010 crime dataset from github a name
crime2010 <- rio::import('https://github.com/RuofanChen/US-crime-data/raw/main/2010-tbl05.xls',sheet = 1, col_names = T,range = "A4:M507")
...
length(is.na(crime_total_df$`Violent crime1`)==TRUE)
#check states
length(table(crime_NonMetro_df$State))
length(table(crime_Metro_df$State))
length(table(crime_Between_df$State))
length(table(crime_total_df$State))
###########################Combining Datasets
...
Year<-rep(2010,dim(crime_dfbystate)[1])
crime_df2010 <- data.frame(Year,crime_dfbystate)
...
length(table(crime_df2010$State))

# check number of rows, recheck in line 138
dim(crime_df2010)

# remove number in state name
crime_df2010$State <- gsub('[[:digit:]]+', '', crime_df2010$State)
# remove , in state name
crime_df2010$State <- gsub('[[:punct:]]+', '', crime_df2010$State)

# Check names
names(crime_df2010)

# give new names for columns
names(crime_df2010) <- c('year','state','area','report_type','actual_rate','violent_crime',
                         'murder','rape','robbery','assault','property_crime',
                         'burglary','larceny_theft','vehicle_theft','population')

# Order the Column's name
crime_df2010 <- crime_df2010[,c('year','state','area','population','actual_rate','report_type',
...
u_crime_df2010 <- gather(crime_df2010,crime_type,number,violent_crime,property_crime,
                         murder,rape,robbery,assault,burglary,larceny_theft,vehicle_theft)

# check number of state
length(table(u_crime_df2010$state))

# recheck number of rows for each crime type  
table(u_crime_df2010$crime_type)

# output the csv file
write.csv(u_crime_df2010,'../../data/1995-2019_reorganized_data/crime_2010.csv')
